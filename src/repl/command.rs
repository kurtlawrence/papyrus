...
pub struct CommandActionArgs<'a> {
	/// The current `Repl` data instance.
	pub repl: Repl<ManualPrint>,
	/// The argument string of the command.
	pub arg: &'a str,
}

type CommandAction = fn(args: CommandActionArgs) -> Result<Repl<Print>, ()>;
...
mod tests {
	...
	#[test]
	fn test_clone() {
		let c = Command::new("test", CmdArgs::None, "Test help", |_| Err(()));
		let clone = c.clone();
