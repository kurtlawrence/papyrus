...
type CommandAction<T> =
	for<'data> fn(Repl<'data, ManualPrint, T>, &str) -> Result<Repl<'data, Print, T>, ()>;

/// A command definition.
pub struct Command<Term: Terminal> {
	/// The command name.
	pub name: &'static str,
...
impl<Term: Terminal> Command<Term> {
	/// Create a new `Command`.
	pub fn new(
		name: &'static str,
		arg_type: CmdArgs,
		help: &'static str,
		action: CommandAction<Term>,
	) -> Self {
		Command {
			name: name,
			arg_type: arg_type,
...
impl<Term: Terminal> Clone for Command<Term> {
	fn clone(&self) -> Self {
		Command {
			name: self.name,
...
pub trait Commands<Term: Terminal> {
	...
	fn find_command(&self, command: &str) -> Result<Command<Term>, String>;
}

impl<Term: Terminal> Commands<Term> for Vec<Command<Term>> {
	fn build_help_response(&self, command: Option<&str>) -> String {
		let mut ret = String::new();

		let write_cmd_line = |cmd: &Command<Term>, str_builder: &mut String| {
...
mod tests {
	...
	#[test]
	fn test_clone() {
		let c = Command::new("test", CmdArgs::None, "Test help", |_, _| Err(()));
		let clone = c.clone();
	...
	fn help_response_types() {
		let cmds = vec![Command::new("test", CmdArgs::None, "Test help", |_, _| {
			Err(())
		})];
		assert_eq!(
		...
		let cmds = vec![Command::new("test", CmdArgs::Text, "Test help", |_, _| {
			Err(())
		})];
		assert_eq!(
			cmds.build_help_response(Some("test")),
		...
		let cmds = vec![Command::new("test", CmdArgs::Expr, "Test help", |_, _| {
			Err(())
		})];
		assert_eq!(
			cmds.build_help_response(Some("test")),
